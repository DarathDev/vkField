...
planSimulation_odin :: proc(settings: ^SimulationSettings, transmitElements: []Element, receiveElements: []Element, scatters: []Scatter) {
	...
	settings.sampleCount = max(i32(math.ceil(((maxDistance - minDistance) / settings.speedOfSound) * settings.samplingFrequency)), 2)
...
findDistanceLimits :: proc(transmitElements: []Element, receiveElements: []Element, scatters: []Scatter) -> (minDistance: f32, maxDistance: f32) {
	...
	for scatter in scatters {
		for transmit in transmitElements {
			delta := linalg.abs(scatter.position - transmit.aperture.rectangle.position)
			elementDelta := linalg.abs([3]f32{transmit.aperture.rectangle.size.x / 2, transmit.aperture.rectangle.size.y / 2, 0})
			minTransmitDistance = min(minTransmitDistance, linalg.length2(delta - elementDelta))
			maxTransmitDistance = max(maxTransmitDistance, linalg.length2(delta + elementDelta))
		}

		for receive in receiveElements {
			delta := linalg.abs(scatter.position - receive.aperture.rectangle.position)
			elementDelta := linalg.abs([3]f32{receive.aperture.rectangle.size.x / 2, receive.aperture.rectangle.size.y / 2, 0})
			minReceiveDistance = min(minReceiveDistance, linalg.length2(delta - elementDelta))
			maxReceiveDistance = max(maxReceiveDistance, linalg.length2(delta + elementDelta))
		}
	}

	minDistance = linalg.sqrt(minTransmitDistance) + linalg.sqrt(minReceiveDistance)
	maxDistance = linalg.sqrt(maxTransmitDistance) + linalg.sqrt(maxReceiveDistance)
	return
}
